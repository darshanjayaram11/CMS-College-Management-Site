...
exports.getAddFee = async (req, res) => {
  try {
    ...
    // Fetch the students for the dropdown
    const studentsSql = 'SELECT s_id, s_name FROM student';
    const students = await queryParamPromise(studentsSql);
    res.render('Admin/fee/addFee', { errors, students });
  } catch (error) {
    console.error('Error fetching students:', error);
    req.flash('error', 'Error fetching student data');
    res.render('Admin/fee/addFee', { errors: [{ msg: 'Error fetching student data' }] });
  }
};

exports.postAddFee = async (req, res, next) => {
  const { fee_type, receipt_no, date, s_id, amount } = req.body;

  // Validate input
  let errors = [];
  if (!fee_type || !receipt_no || !date || !s_id || !amount) {
    errors.push({ msg: 'All fields are required' });
  }

  if (errors.length > 0) {
    res.render('Admin/fee/addFee', { errors, page_name: 'fee' });
  } else {
    try {
      const sql = 'INSERT INTO fee (fee_type, reciept_no, date, s_id, amount) VALUES (?, ?, ?, ?, ?)';
      await queryParamPromise(sql, [fee_type, receipt_no, date, s_id, amount]);
      req.flash('success_msg', 'Fee added successfully');
      res.redirect('/admin/dashboard'); 
    } catch (error) {
      req.flash('error', 'Error adding fee');
      res.render('Admin/fee/addFee', { errors: [{ msg: 'Error adding fee' }], page_name: 'fee' });
    }
  }
};
...
exports.getFee = async (req, res) => {
  try {
    ...
    const fees = await queryParamPromise(feesSql);

    const messages = req.flash('info') || [];

    res.render('Admin/fee/getFee', { fees, messages, page_name: 'fee' });
  } catch (error) {
    console.error('Error fetching fee records:', error);
    req.flash('error', 'Error fetching fee records');
    res.render('Admin/fee/getFee', { fees: [], messages: req.flash('error'), page_name: 'fee' });
  }
};
...
exports.postAddStudent = async (req, res, next) => {
  ...
  // Query to get the current section count and value
  const sql1 = `
