...
exports.addAssignment = async (req, res, next) => {
  const { name, deadline, class_id } = req.body;

  // Validate the input data
  if (!name || !deadline || !class_id) {
      return res.status(400).send('Missing required fields');
  }

  // Insert assignment into the database
  const sql = 'INSERT INTO assignment (name, deadline, class_id) VALUES (?, ?, ?)';
  try {
      await queryParamPromise(sql, [name, deadline, class_id]);
      res.redirect('/staff/view-assignments');
  } catch (error) {
...
exports.viewAssignments = async (req, res, next) => {
  try {
    ...
    // Updated query to fetch assignments along with the class name
    const sql = `
      SELECT 
        assignment.*, 
      ...
      INNER JOIN 
        course ON class.c_id = course.c_id
      WHERE 
        class.st_id = ?
    `;

    // Execute the query with the staffId as the parameter
    const assignments = await queryParamPromise(sql, [staffId]);

    res.render('Staff/viewAssignments', {
      assignments,
      page_name: 'view-assignments',
    });
  } catch (error) {
...
exports.forgotPassword = async (req, res, next) => {
  ...
  const data = {
    ...
    html: `<h2>Please click on given link to reset your password</h2>
                <p>${process.env.URL}/staff/resetpassword/${token}</p>
